...
class WordApplicationBuilder:
    ...
    def _eligibility_source_line(self) -> str:
        ...
        base = (
            f"ELIGIBILITY: CDFI Fund NMTC Eligibility Table using {ACS_VINTAGE}. "
            "Tract eligibility determined by Low Income Community (LIC) "
            "criteria per IRC §45D."
        )
    ...
    def _build_methodology(self, doc: Document) -> None:
        ...
        p = doc.add_paragraph(
            ...
            # Every figure below is composed from nmtcapp/data/ by
            # renderers/_methodology, not typed here. Three hand-typed copies
            # of this appendix drifted from each other and from the constants
            # they described; see that module's docstring.
            f"{distress_definitions()} {noaa_note()}\n\n"
