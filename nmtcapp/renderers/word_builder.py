...
class WordApplicationBuilder:
    ...
    def _build_methodology(self, doc: Document) -> None:
        ...
        p = doc.add_paragraph(
            ...
            "workbook, 2016-2020 ACS, columns 14 and 15 — the file this tool "
            "loads to classify every tract): Severe distress = LIC AND "
            "(Poverty>30%; MFI<=60%; Unemployment>=1.5). Deep distress = LIC AND "
            "(Poverty>40%; MFI<=40%; Unemployment>=2.5). The semicolons are ORs; "
            "both tiers additionally require the tract to be a Low-Income "
            "Community. NMTC rounds are announced by a NOAA — Notice of "
            "Allocation Availability — and the CY 2026 NOAA is not yet "
            "published.\n\n"
            ...
            "IMPACT SCREENING BANDS: THIS TOOL'S OWN BANDS (5 / 12 / 20 FTE per "
            "$1MM QEI). The CDFI Fund publishes no jobs-per-QEI benchmark in any "
            "denominator: it reports job counts and dollar counts separately and "
            "never divides them, and it publishes no distribution, so no band here "
            "is a percentile. These are not a federal figure and must not be cited "
            "as one.\n\n"
