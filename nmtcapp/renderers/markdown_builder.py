...
class MarkdownApplicationBuilder:
    ...
    def _methodology_note(self) -> str:
        ...
        if degraded:
            ...
            eligibility_line = (
                f"- Eligibility data: CDFI Fund NMTC Eligibility Table, "
                f"{ACS_VINTAGE} — {len(ids)} of {pr.total_projects} projects "
                f"unverified (no census tract assigned): {', '.join(ids)}"
            )
        else:
            eligibility_line = (
                f"- Eligibility data: CDFI Fund NMTC Eligibility Table, {ACS_VINTAGE}"
            )

        # Every figure below comes from nmtcapp/data/ via
        # renderers/_methodology. It used to be typed here, and typed again in
        # word_builder and pdf_builder, and the three copies disagreed.
        return (
            f"## Methodology Note\n\n"
            f"Generated by **nmtc-application-builder v{__version__}** on {date.today().isoformat()}.\n\n"
            f"{eligibility_line}\n"
            f"- {deal_economics_note()}\n"
            f"- {impact_bands_note()}\n"
            f"- {readiness_weights_note()} {noaa_note()}\n"
...
def _df_to_md(df, max_rows: int = 50) -> str:
    ...
    for _, row in display.iterrows():
        # Format by column, not by magnitude: this branch printed "0.328" for a
        # share stored as a fraction and gave every large float two decimals
        # with no currency marker. See renderers/_cell_format.
        cells = [
            format_cell(col, v).replace("|", "\\|")
            for col, v in zip(display.columns, row)
        ]
