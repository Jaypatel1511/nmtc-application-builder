...
from nmtcapp.renderers._document_properties import generator_stamp, stamp_xlsx
...
class ExcelApplicationBuilder:
    ...
    def build(self) -> "Workbook":
        ...
        # openpyxl's default says creator "openpyxl" and no title.
        stamp_xlsx(wb, self.application)
        return wb
    ...
    def _build_summary_dashboard(self, wb: "Workbook") -> None:
        ...
        # THE VERSION IS ON THE WORKBOOK NOW (1.7.1 R6). Markdown, Word and
        # PDF each said "generated by nmtc-application-builder vX.Y.Z"; this
        # footer said only "Generated <date>", so the one format most likely
        # to be forwarded could not be told stale from fresh — the read that
        # caught both the 1.6.5 and the 1.7.0 P0 on the other three. Read
        # from the package, never typed.
        ws.cell(row=footer_row, column=1,
                value=f"CONFIDENTIAL — {app.cde.name} — "
                      f"{nmtc_round_phrase(app.application_round)} — "
                      f"Generated {date.today().isoformat()} by {generator_stamp()}")
